...
namespace Aplicacion_Web2.Controllers
{
    ...
    public class ConsultaMedicaController: ControllerBase
    {
        ...
        [HttpGet("custom")]
        public async Task<ActionResult<List<Consulta_Medica>>> findAllCustom()
        {
            return await context.Consulta_Medica.Where(x => x.estadoConsultaMedica == true).ToListAsync(); //Cambiar estado(traer true o false)
        }
